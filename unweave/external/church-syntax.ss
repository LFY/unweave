(library (unweave external church-syntax)
         (export

           var?  var<?

           ref?  ref->lab ref->var explode-ref

           lambda?  lambda<?  lambda->lab lambda->formals lambda->call lambda->return-type exp<? has-type-annotation?  explode-lambda

           closure? factor-closure?  closure->lambda closure->env explode-closure

           call?  call->lab call->fun call->args explode-call

           primop?  primop->lab primop->fun primop->args explode-primop

           if?  if->lab if->cond if->then if->else explode-if

           letrec? letrec->lab letrec->bindings letrec->body explode-letrec

           xrp? xrp->lab xrp->scorer xrp->name xrp->prop-fx-name xrp->params explode-xrp
           xrps? xrps->lab xrps->scorer xrps->name xrps->prop-fx-name xrps->params explode-xrps
           xrp+init? xrp+init->lab xrp+init->scorer xrp+init->name xrp+init->prop-fx-name xrp+init->initval xrp+init->params explode-xrp+init           factor? factor->lab factor->formals factor->call explode-factor

           const? const->lab const->val explode-const

           rv? rv->addr rv->name rv->params rv->val 

           assert? assert->lab assert->body explode-assert
           fundep? fundep->lab fundep->x fundep->y

           make-xrp-trace xrp-trace->addr xrp-trace->var xrp-trace->prop-fx-name xrp-trace->params xrp-trace->val xrp-trace->score-fx-name explode-xrp-trace

           quoted?

           all? my-any extend lookup not-found?

           subexpr-walk
           contains?

           get-mean get-var elems-after-n
